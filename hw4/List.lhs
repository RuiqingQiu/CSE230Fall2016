...
\begin{code}
{-@ length :: li : List a -> {len : Nat | len == size li}  @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {l1 : List a | size l1 > 0} -> a @-}
...
{-@ concat :: ll : List (List a)-> {l : List a | size l == size_of_ll ll} @-}
concat Emp = Emp
concat (Emp :+: xs) = concat xs
concat ((x :+: xs) :+: remaining_ll) = x :+: (concat (xs :+: remaining_ll))
