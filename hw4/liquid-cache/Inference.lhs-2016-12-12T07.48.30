...
ti env (ESnd e)       =
      do
              ...
              s2 <- mgu t1 (tv1 `TCom` tv2)
              let s3 = s2 `after` s1
              return (s3, apply s2 tv2)

ti env ENil            =
      do
        tv1 <- freshTVbl "a"
        return (empSubst, TList tv1)
...
ti env (EDcons e)      =
  do
      ...
      tv1 <- freshTVbl "a"
      s2 <- mgu t1 (TList tv1)
      return (s2 `after` s1, TCom (apply s2 tv1) (apply s2 t1))

ti env (ERec x e1 e2)  =
  do
    tv1 <- freshTVbl "a"
    let env1 = env \\ (x, Forall [] tv1)
    (s1, t1) <- ti env1 e1
    s2 <- mgu (apply s1 tv1) t1
    let s3 = s2 `after` s1
    let t2 = generalize (apply s3 env) (apply s3 t1)
    let env2 = env \\ (x, t2)
    (s4, t3) <- ti (apply s3 env2) e2
    return (s4 `after` s3, t3)
